...
function Restaurant(resName, resAddress, resMenu){
    ...
    this.information = function(){
        ...
        return `${this.resName}/[${this.resMenu}]/[${this.resMenu}] / ${this.resAddress}`;
    }
}

/* 문자열.split("구분자") */
//  -> 문자열을 " 구분자" 로 쪼개서 매열로  반환
...
/* 화면에 존재하는 요소들 얻어오기 (buttin, input) */
...
addBtn.addEventListener("click", () => {
    ...
    resList.push(new Restaurant(resName.value, 
        resAddress.value, resMenu.value));

    // 이전 input에 작성된 내용을 삭제
    resName.value     = "";
    resAddress.value  = "";
    resMenu.value     = "";
...
selectBtn.addEventListener("click", () => {
    ...
    //#printList 의 이전 냐용을 모두 삭제
    printList.innerHTML = "";


    //resList 의 요소를 에용해 li요소를 만들어 추가
