...
function check3(){
    ...
    // 배열의 index는 0부터 배열 길이 -1 까지 증가하느 규칙성이 있다
...
function selectMenu(){
    ...
    // 점심 메뉴로 초기회된 배열 생성
    const menus = ["굶기","김밥","제육볶음","돈까스","파스타", "샐러드",
                    "라면", "햄버거", "순대국밥", "닭갈비"];
...
/* 주문하기 프로그램 */
/* 
    -주문하기 버튼 클릭 시 prompt 를 이용해서
     메뉴명, 수량 입력 받기

    -메뉴명 입력 시 취소를 클릭하면 "주문 완료"

    -슈량 입력 시 취소를 클릭하면 해당 메뉴 주문만 취소
*/
function orderFn(){
    ...
    while(true){ //무한 반복
        ...
        // -> 배열 검색
        // 1) for 문 이용
        //let idx = -1; //일치하는 메뉴에 인덱스를 저장할 변수
        ...
        let input = prompt("수랭 입력");

        if(input == null){ //수량 입력을 취소 -> 다시 메뉴 입력
            continue;
        }
...
function check6(){
    ...
    const arr = []; //1차원 배열(요소로 1차원 배열 대입)
